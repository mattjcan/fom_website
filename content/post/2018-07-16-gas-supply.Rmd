---
title: "To get cheaper gas we need to develop more gas"
author: "Matt Canavan"
date: '2018-07-16'
slug: gas-supply
categories: [articles]
tags: []
excerpt: "The government's action to return more gas to the domestic market has helped lower gas prices. For further reductions in gas prices bans on gas development must be removed."
output:
  tufte::tufte_html: 
    fig_height: 4  
---
...
```{r }
p_aust$Sydney + scale_color_manual(values = c("grey", "#2166ac"), labels = c("North East Asia", "Sydney")) 
```

```{r }
p_aust$Victoria + scale_color_manual(values = c("grey", "#2166ac"), labels = c("North East Asia", "Victoria"))  
```
...
```{r }
grid.arrange(p_gap$vic_bris + theme_blog, p_gap$vic_asia + theme_blog, ncol = 2) 
```
